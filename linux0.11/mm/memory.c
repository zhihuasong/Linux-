...
#define MAP_NR(addr) (((addr)-LOW_MEM)>>12)	// 将指定物理内存地址映射为页号。
#define USED 100							// 页面被占用标志。

// 该宏用于判断给定地址是否位于当前进程的代码段中。[??]是否存在bug，
// 可能指向addr所在页面的首地址，也可能指向addr所在页面的下一个页面的首地址。
...
// 输入：%1(ax=0) - 0；%2(LOW_MEM)（内存低端，见上面定义）；%3(cx=PAGING PAGES)；%4(edi=mem_map+PAGING_PAGES-1)。
// 输出：返回%0(ax=页面起始地址)。
...
{
register unsigned long __res asm("ax");	// 定义一个寄存器变量，寄存在ax寄存器

__asm__("std ; repne ; scasb\n\t"	// 方向位置位，将al(0)与对应每个页面(di)的内容比较
	"jne 1f\n\t"					// 如果没有等于0 的字节，则跳转结束（返回0）。
	"movb $1,1(%%edi)\n\t"			// 将对应页面的内存映像位置1。
	"sall $12,%%ecx\n\t"			// 页面数*4K = 相对页面起始地址，sall算术左移
	"addl %2,%%ecx\n\t"				// 再加上低端内存地址，即获得页面实际物理起始地址
	"movl %%ecx,%%edx\n\t"
	"movl $1024,%%ecx\n\t"			// 寄存器ecx 置计数值1024。
	"leal 4092(%%edx),%%edi\n\t"	// 将4092+edx 的位置 -> edi(该页面的末端,指向该页面末端最后4字节的低地址)
...
int free_page_tables(unsigned long from,unsigned long size)
{
	...
	if (from & 0x3fffff)				// 要释放内存块的地址需以4M 为边界。（这里指线性地址）
	...
	// 物理地址0 开始，因此实际的目录项指针=目录项号<<2，也即(from>>20)。与上0xffc（111111111100B） 确保
	// 目录项指针范围有效。
	dir = (unsigned long *) ((from>>20) & 0xffc); /* _pg_dir = 0 */
	for ( ; size-->0 ; dir++) {			// size 现在是需要被释放内存的目录项数。
		if (!(1 & *dir))				// 如果该目录项无效(P 位=0)，则继续。目录项的位0(P 位)表示对应页表是否存在
			continue;
		pg_table = (unsigned long *) (0xfffff000 & *dir);	// 取目录项中页表地址，在4K边界，后12位必为0
		for (nr=0 ; nr<1024 ; nr++) {	// 循环free当前页表所指向的1024个页面内存空间
			if (1 & *pg_table)
				free_page(0xfffff000 & *pg_table);
			*pg_table = 0;
			pg_table++;
		}
		free_page(0xfffff000 & *dir);	// free当前页表内存空间
		*dir = 0;						// 对相应页表的目录项清零。
	}
 ...
 * 再调试这块代码了 :-)
 *
 * 注意！我们并不是仅复制任何内存块- 内存块的地址需要是4Mb的倍数（正好
 ...
 */
// 把一物理内存页面映射到指定的线性地址处。
// 主要工作是在页目录和页表中设置指定页面的信息。若成功则返回页面物理地址。
// page 为物理地址，address 为线性地址
...
/* NOTE !!! This uses the fact that _pg_dir=0 */
	...
	// 否则，申请空闲页面给页表使用，并在对应目录项中置相应标志7（User, U/S, R/W）。然后将该页表的地址 -> page_table。
	else {
...
void un_wp_page(unsigned long * table_entry)
{
	...
	old_page = 0xfffff000 & *table_entry;	// 取原页表项对应页面的首地址。
	// 如果原页面地址大于内存低端LOW_MEM(1Mb)，并且其在页面映射字节图数组中值为1（表示仅
	// 被引用1 次，页面没有被共享，此处可能是对引用同一个共享页面的第2个进程取消写时保护的处理），
	// 则在该页面的页表项中置R/W 标志（可写），并刷新页变换高速缓冲，然后返回。
...
// 参数error_code 是由CPU 自动产生并压栈；address 是页面线性地址。
// 写共享页面时，需复制页面（写时复制）。
...
// 取得一页空闲内存并映射到指定线性地址处，即在指定页目录和页表处登记该页面
...
// 参数address 是期望共享的某页面相对于p进程数据空间的地址。
// 返回1-成功，0-失败。
...
/* is the page clean and present? */
	...
	if (phys_addr >= HIGH_MEMORY || phys_addr < LOW_MEM)
		return 0;
	// 对当前进程页面进行操作。取页目录项内容 -> to。如果该目录项无效(P=0)，则取空闲页面，并更新to_page 所指的目录项。
 ...
 * 期望共享的某页面相对于当前数据空间的地址。
 *
 * 首先我们通过检测executable->i_count 来查证是否可行。如果有其它任务已共享
 * 该inode，则它应该大于1。
 */
// 共享页面。在缺页处理时看看能否共享页面
// 参数address 是期望共享的某页面相对于当前数据空间的地址。
// 返回1 - 成功，0 - 失败。。
...
void mem_init(long start_mem, long end_mem)
{
	...
	for (i=0 ; i<PAGING_PAGES ; i++)	// 先对所有的内存页面使用次数初始化为USED，USED被定义为100
		mem_map[i] = USED;
	i = MAP_NR(start_mem);
	end_mem -= start_mem;
	end_mem >>= 12;
	while (end_mem-->0)					// 再将主内存区所有页面的使用次数清0，系统以后把使用次数为0的页面视为空闲页面
		mem_map[i++]=0;
}
