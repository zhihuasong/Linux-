...
extern int end;	// end为由链接器ld生成的表明内核末端的变量，end的地址就是内核后第一个内存单元的地址，
				// 即，end变量就在内核后第一个内存单元处。
...
{
	struct buffer_head * h = start_buffer;	// start_buffer = &end,参见本文件最上面
	void * b;
	int i;
	...
	if (buffer_end == 1<<20)
		...
		b = (void *) buffer_end;
	while ( (b -= BLOCK_SIZE) >= ((void *) (h+1)) ) {	// BLOCK_SIZE 被定义为：1024
		h->b_dev = 0;
		h->b_dirt = 0;
		h->b_count = 0;
		h->b_lock = 0;
